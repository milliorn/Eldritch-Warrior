...
namespace EldritchWarrior.Source.Shifter
{
			...
			while (GetIsItemPropertyValid(ip)) // Loop through all the item properties
			{
				if (GetIsStackingProperty(ip)) // See if it's a stacking property
                {
                    ...
                    else if (GetItemPropertyType(ip) == ItemPropertyType.DecreasedAbilityScore)
                    {
                        ...
                        var modifyType = ItemPropertyArmorClassModifierType.Dodge;
                        var damageType = ACType.VsDamageTypeAll;
                        var childEffect = EffectACDecrease(value, modifyType, damageType);
        ...
        uint oldBoots, uint oldBelt, uint oldHelmet, uint oShield, uint oWeapon, uint oHideOld)
    {
			...
			if (GetIsObjectValid(oldWeapon) && GetIsObjectValid(newWeapon))
			{
				...
				else if (GetWeaponRanged(oldWeapon) && GetWeaponRanged(newWeapon))
				{
					...
					while (GetIsItemPropertyValid(ip))
					{
                        ...
                        IPRemoveMatchingItemProperties(newWeapon, ItemPropertyType.UnlimitedAmmunition, DurationType.Permanent);
                        AddItemProperty(DurationType.Permanent, ip, newWeapon);
                        DestroyObject(ammo);
                    }
		...
		public static bool GetIsCreatureWeapon(uint item) => GetBaseItemType(item) == BaseItemType.CreatureBludgeWeapon ||
